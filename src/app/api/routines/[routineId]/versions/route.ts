...
import { routineVersionInputSchema } from "@/features/routines/schemas/routine.schema";
...
export async function POST(request: NextRequest, { params }: Context) {
  try {
    return NextResponse.json(
      {
        data: await routineService.addVersion(
          objectIdSchema.parse((await params).routineId),
          routineVersionInputSchema.parse(await request.json()),
        ),
      },
      { status: 201 },
    );
